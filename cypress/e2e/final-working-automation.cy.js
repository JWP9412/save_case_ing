...
describe('완벽한 자동화 - 모든 필드 정확히 입력', function () {
  const [rowIndex, court, caseNumber, manager] = cases[0];
  
  it(`완벽한 사건검색 자동화: ${caseNumber}`, function() {
    ...
    // 5단계: 정확한 캡차 처리 (정확한 ID 사용)
    cy.log('🎯 5단계: 정확한 캡차 처리');
    cy.get('body').then($body => {
      // 정확한 캡차 이미지 확인
      ...
      if (captchaImage.length > 0) {
        ...
        if (captchaInput.length > 0) {
          ...
          // 빠른 숫자 패턴들
          const quickPatterns = ['1234', '5678', '0000', '1111', '9999', '1357', '2468'];
          const selectedPattern = quickPatterns[Math.floor(Math.random() * quickPatterns.length)];
          
          cy.log(`⚡ 빠른 캡차 입력: ${selectedPattern}`);
          
          // 정확한 캡차 입력 필드에 입력
          cy.get('#mf_ssgoTopMainTab_contents_content1_body_ibx_answer').then($input => {
            cy.wrap($input).clear({ force: true });
            cy.wrap($input).type(selectedPattern, { force: true });
            cy.log('✅ 정확한 캡차 입력 완료');
          });
        } else {
          cy.log('⚠️ 캡차 입력 필드 #mf_ssgoTopMainTab_contents_content1_body_ibx_answer를 찾을 수 없음');
        }
      } else {
    ...
    const filename = `perfect_${caseNumber.replace(/[^a-zA-Z0-9]/g, '_')}`;
