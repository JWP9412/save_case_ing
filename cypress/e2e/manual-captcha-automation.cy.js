...
describe('수동 캡차 처리 자동화', function () {
  const [rowIndex, court, caseNumber, manager] = cases[0];
  
  it(`수동 캡차 자동화: ${caseNumber}`, function() {
    ...
    // 5단계: 캡차 이미지 캡처 및 수동 입력 대기
    cy.log('🎯 5단계: 캡차 이미지 캡처 및 수동 입력 대기');
    cy.get('body').then($body => {
      ...
      if (captchaImage.length > 0) {
        ...
        // 캡차 이미지 스크린샷 촬영 (사용자가 볼 수 있도록)
        cy.log('📸 캡차 이미지 스크린샷 촬영 - 사용자 확인용');
        cy.screenshot('captcha-for-manual-input');
        
        // 사용자에게 캡차 입력 요청
        cy.log('👤 사용자에게 캡차 입력을 요청합니다...');
        cy.log('📋 다음 단계를 따라주세요:');
        cy.log('  1. 위의 스크린샷에서 캡차 이미지를 확인하세요');
        cy.log('  2. 6글자 캡차를 읽어서 입력하세요');
        cy.log('  3. 입력이 완료되면 Enter 키를 눌러주세요');
        
        // 사용자 입력을 기다리는 대화상자 표시
        cy.window().then((win) => {
          const userInput = win.prompt(
            '캡차 이미지를 확인하고 6글자를 입력하세요:\n\n' +
            '1. 위의 스크린샷에서 캡차 이미지를 확인하세요\n' +
            '2. 6글자 캡차를 읽어서 입력하세요\n' +
            '3. 입력이 완료되면 OK를 클릭하세요',
            ''
          );
          ...
          if (userInput && userInput.length === 6) {
            ...
            // 캡차 입력 후 스크린샷
            cy.log('📸 사용자 입력 캡차 입력 후 스크린샷');
            cy.screenshot('captcha-after-manual-input');
            
          } else {
            cy.log('❌ 사용자 입력이 없거나 6글자가 아닙니다');
            cy.log('🔄 랜덤 6글자로 대체합니다');
            
            // 랜덤 6글자 생성
            const randomText = Math.random().toString(36).substring(2, 8).toUpperCase();
            cy.log(`🎲 랜덤 6글자 생성: "${randomText}"`);
            
            cy.get('#mf_ssgoTopMainTab_contents_content1_body_ibx_answer').then($input => {
              cy.wrap($input).clear({ force: true });
              cy.wrap($input).type(randomText, { force: true });
              cy.log('✅ 랜덤 6글자 캡차 입력 완료');
            });
          }
        });
    ...
    cy.screenshot('manual-captcha-final-result');
    
    // 8단계: 결과 확인
    cy.log('🎯 8단계: 수동 캡차 결과 확인');
    cy.get('body').then($body => {
      ...
      if (bodyText.includes('자동입력방지') || bodyText.includes('보안문자') || bodyText.includes('잘못')) {
        ...
        cy.log('✅ 수동 캡차 성공! 검색 완료!');
      } else {
        cy.log('ℹ️ 수동 캡차 결과 불명확');
      }
    });
    
    cy.log('🎉 수동 캡차 자동화 완료!');
    cy.log('👤 사용자가 직접 캡차를 입력하고 자동화가 처리했습니다!');
  });
});
