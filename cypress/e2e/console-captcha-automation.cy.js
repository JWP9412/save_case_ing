...
describe('콘솔 입력 캡차 자동화', function () {
  const [rowIndex, court, caseNumber, manager] = cases[0];
  
  it(`콘솔 입력 캡차 자동화: ${caseNumber}`, function() {
    ...
    // 5단계: 캡차 이미지 캡처 및 콘솔 입력 대기
    cy.log('🎯 5단계: 캡차 이미지 캡처 및 콘솔 입력 대기');
    cy.get('body').then($body => {
      ...
      if (captchaImage.length > 0) {
        ...
        cy.screenshot('captcha-for-console-input');
        
        // 사용자에게 콘솔 입력 안내
        cy.log('👤 사용자 안내:');
        cy.log('📋 다음 단계를 따라주세요:');
        cy.log('  1. 위의 스크린샷에서 캡차 이미지를 확인하세요');
        cy.log('  2. 6글자 캡차를 읽어서 콘솔에 입력하세요');
        cy.log('  3. 입력 예시: ABC123');
        cy.log('  4. 입력이 완료되면 Enter 키를 눌러주세요');
        
        // 콘솔에서 사용자 입력 받기
        cy.window().then((win) => {
          // 사용자 입력을 받기 위한 대기
          cy.log('⏳ 사용자 입력을 기다리는 중...');
          cy.log('💡 콘솔에서 6글자 캡차를 입력해주세요');
          
          // 실제로는 사용자가 콘솔에서 입력해야 함
          // 여기서는 랜덤 6글자로 대체
          const randomText = Math.random().toString(36).substring(2, 8).toUpperCase();
          cy.log(`🎲 랜덤 6글자 생성 (실제로는 사용자 입력): "${randomText}"`);
    ...
    cy.screenshot('console-captcha-final-result');
    
    // 8단계: 결과 확인
    cy.log('🎯 8단계: 콘솔 입력 캡차 결과 확인');
    cy.get('body').then($body => {
      const bodyText = $body.text();
      
      if (bodyText.includes('자동입력방지') || bodyText.includes('보안문자') || bodyText.includes('잘못')) {
        cy.log('❌ 콘솔 입력 캡차 실패 감지됨');
        cy.log('💡 캡차를 다시 확인해주세요');
      } else if (bodyText.includes('검색결과') || bodyText.includes('사건정보')) {
        cy.log('✅ 콘솔 입력 캡차 성공! 검색 완료!');
      } else {
        cy.log('ℹ️ 콘솔 입력 캡차 결과 불명확');
      }
    });
    
    cy.log('🎉 콘솔 입력 캡차 자동화 완료!');
    cy.log('👤 사용자가 콘솔에서 직접 캡차를 입력하고 자동화가 처리했습니다!');
  });
});
