...
describe('간단한 6글자 캡차 테스트', function () {
  const [rowIndex, court, caseNumber, manager] = cases[0];
  
  it(`6글자 캡차 테스트: ${caseNumber}`, function() {
    ...
    cy.get('body').then($body => {
      ...
      if (captchaImage.length > 0) {
        ...
        // 캡차 이미지 스크린샷 촬영
        cy.log('📸 6글자 캡차 이미지 스크린샷 촬영');
        cy.screenshot('6digit-captcha-image');
        
        // 6글자 캡차 패턴들 (숫자+문자 조합)
        const sixDigitPatterns = [
          '123456', 'ABCDEF', '1A2B3C', 'A1B2C3',
          '111111', 'AAAAAA', '1A1A1A', 'A1A1A1',
          '000000', 'ZZZZZZ', '0Z0Z0Z', 'Z0Z0Z0',
          '999999', 'YYYYYY', '9Y9Y9Y', 'Y9Y9Y9'
        ];
        
        const selectedPattern = sixDigitPatterns[Math.floor(Math.random() * sixDigitPatterns.length)];
        cy.log(`🎲 선택된 6글자 패턴: "${selectedPattern}"`);
        ...
        cy.get('#mf_ssgoTopMainTab_contents_content1_body_ibx_answer').then($input => {
          ...
          cy.wrap($input).then($updated => {
            ...
            cy.log(`✅ 6글자 캡차 입력 완료: "${inputValue}"`);
            
            if (inputValue === selectedPattern) {
              cy.log('✅ 6글자 캡차 정확히 입력됨!');
            } else {
              cy.log(`⚠️ 6글자 캡차 불일치: 예상 "${selectedPattern}", 실제 "${inputValue}"`);
            }
          });
        });
        
        // 캡차 입력 후 스크린샷
        cy.log('📸 6글자 캡차 입력 후 스크린샷');
        cy.screenshot('6digit-captcha-after-input');
        
      } else {
        cy.log('ℹ️ 6글자 캡차 이미지가 없음');
      }
    });
    ...
    cy.screenshot('6digit-final-result');
    
    // 8단계: 결과 확인
    cy.log('🎯 8단계: 6글자 캡차 결과 확인');
    cy.get('body').then($body => {
      const bodyText = $body.text();
      
      if (bodyText.includes('자동입력방지') || bodyText.includes('보안문자') || bodyText.includes('잘못')) {
        cy.log('❌ 6글자 캡차 실패 감지됨');
        cy.log('💡 OCR이 필요할 수 있습니다');
      } else if (bodyText.includes('검색결과') || bodyText.includes('사건정보')) {
        cy.log('✅ 6글자 캡차 성공! 검색 완료!');
      } else {
        cy.log('ℹ️ 6글자 캡차 결과 불명확');
      }
      
      // 캡차 관련 메시지 확인
      if (bodyText.includes('6자')) {
        cy.log('📝 페이지에서 "6자" 관련 메시지 발견');
      }
      if (bodyText.includes('자동입력 방지 문자 6자를 입력하십시오')) {
        cy.log('📝 정확한 6글자 캡차 안내 메시지 발견');
      }
    });
    
    cy.log('🎉 6글자 캡차 테스트 완료!');
    cy.log('💡 OCR이 필요하다면 별도 OCR 서비스나 API를 사용해야 할 수 있습니다');
  });
});
