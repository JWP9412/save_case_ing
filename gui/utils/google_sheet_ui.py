...
def load_case_list_cache():
    """data/case_list_cache.json에서 사건 목록 캐시 로드. 유효한 리스트면 list[dict], 없거나 오류 시 None."""
    path = _case_list_cache_path()
    try:
        if os.path.isfile(path):
            with open(path, "r", encoding="utf-8") as f:
                data = json.load(f)
...
def save_case_list_cache(data):
    """사건 목록을 data/case_list_cache.json에 저장."""
    path = _case_list_cache_path()
    os.makedirs(os.path.dirname(path) or ".", exist_ok=True)
    with open(path, "w", encoding="utf-8") as f:
...
def _apply_loaded_data_to_app(app, google_data):
    """구글 시트/캐시에서 받은 raw 데이터를 app에 적용 (hidden 필터, 정렬, UI). _on_load_google_sheet_done과 동일한 후처리."""
...
def _on_load_google_sheet_done(app, google_data, spreadsheet, error):
    ...
    save_case_list_cache(google_data)
    _apply_loaded_data_to_app(app, google_data)
    max_limit = getattr(config, "MAX_PARALLEL_LIMIT", 20)
    n_cases = len(app.case_list)
    smart_parallel = max(1, min(n_cases // 2, max_limit))
    app.log_message(
...
def load_google_sheet(app, force_network=False):
    """사건 목록 로드. force_network=False면 캐시 우선(있으면 즉시 적용), 없으면 구글 시트 조회. force_network=True면 항상 구글 시트 조회 후 캐시 갱신."""
    if not force_network:
        cached = load_case_list_cache()
        if cached is not None:
            app.reset_internal_data()
            _apply_loaded_data_to_app(app, cached)
