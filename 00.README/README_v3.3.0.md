...
대법원 나의 사건 조회 자동화 시스템 (Puppeteer + Python GUI) - v3.3.0
...
1. **사건 목록 열 너비 UX (Excel 스타일)** *(v3.3.0 신규)*
   - 드래그 중 가이드라인만 표시하여 렉 없이 리사이즈.
   - 드롭 시 전체 화면을 다시 그리지 않고 열 너비만 즉시 반영.
   - 조절한 열 너비는 `column_widths.json`에 저장되어 다음 실행 시 자동 적용.

2. **캡차 입력란 오타 수정** *(v3.3.0 신규)*
   - 캡차 입력란에서 Backspace/Delete로 글자를 지울 수 있도록 검증 로직을 수정했습니다.

3. **브라우저 표시 설정 분리 (유지보수)** *(v3.3.0 신규)*
   - 실행 시 브라우저 창은 기본적으로 숨김(headless).
   - 프로젝트 루트의 `maintenance.js`에서 `browserHeadless: false`로 변경하면 디버깅 시 브라우저 창을 다시 볼 수 있습니다.
...
자세한 내역: `00.CHANGELOG/CHANGELOG_v3.3.0.md`
