# 프로젝트 구조 (Project Structure) - v4.1.0

## Root Directory
- **`batch_gui_maker.py`**:
  - 메인 GUI 로직. v4.1.0: 표준 로거 초기화(`setup_logger`), GUI 핸들러 등록(`register_gui_handler`), `log_message`가 로거로 위임.
...
- **`config.py`**:
  - 설정 상수. v4.1.0 기준 `APP_VERSION = "4.1.0"`. `COLUMN_WIDTHS_FILE`, `HEADER_IMAGE_PATH`, `USER_SETTINGS_FILE` 등.
...
- **`panels/`** (v4.1.0):
  ...
  - `CaseListPanel`: 사건 목록 제목 "사건 목록(N)", 가로 스크롤 동기화(`_sync_xview(*args)`).
