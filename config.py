...
# 주니어 참고: 지연 일수 = 진행내용 일자 다음날~조회일 중 월~금 개수(토·일 제외)
DELAY_REMARK_MIN_BUSINESS_DAYS = 1
DELAY_REMARK_SUFFIX_FMT = "({n}일 지연 등록. 영업일 기준)"
...
# 시트 로드 시 자동 산정되는 병렬도의 상한 (동시에 뜨는 브라우저 수)
# 사건수//2 로 계산하되 이 값을 넘지 않습니다. 수동으로 설정 창에서 더 올리는 것은 허용.
AUTO_PARALLEL_CAP = 4
...
# ============================================================================
# Node(Puppeteer) 사이트 접속 / 법원 선택 타임아웃·재시도
# ============================================================================
# page.goto 타임아웃 (ms). 동시 접속이 많을 때 30초로는 자주 끊깁니다.
NODE_GOTO_TIMEOUT_MS = 45000
# 사이트 접속 / 법원 선택 재시도 횟수 (최초 시도는 포함하지 않음 → 총 시도 = 1 + N)
NODE_NAV_MAX_RETRY = 2
# 재시도 사이 대기 시간 (ms)
NODE_NAV_RETRY_DELAY_MS = 3000
...
USER_SETTINGS_OVERRIDABLE = (
    ...
    "DEFAULT_MAX_RETRY",
    "DEFAULT_RETRY_DELAY",
    "NODE_GOTO_TIMEOUT_MS",
    "NODE_NAV_MAX_RETRY",
    "NODE_NAV_RETRY_DELAY_MS",
