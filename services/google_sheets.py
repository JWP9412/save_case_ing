...
class GoogleSheetsService:
    ...
    def _get_client(self):
        ...
        if self._client is None:
            ...
            creds = Credentials.from_service_account_file(config.GOOGLE_AUTH_FILE, scopes=scope)
    ...
    def load_case_list(self):
        ...
        try:
            ...
            self._log(f"[INFO] {len(all_data)}개 행 중 {len(filtered_data)}개 유효한 데이터 로드 완료")
    ...
    def save_progress_data(self, case, result_data, log_callback=None):
        ...
        try:
            ...
            self._log(f"💾 [DEBUG] result_data 타입: {type(result_data)}, 길이: {len(result_data) if isinstance(result_data, list) else 'N/A'}")
                ...
                if len(all_values) > 0:
                    ...
                    for row in reversed(all_values):
                        if len(row) >= 2 and row[0] == '업데이트 일시' and row[1]:
                            ...
                            self._log(f"💾 [DEBUG] 이전 업데이트 시간: {previous_update_time}")
                            break
            except Exception as e:
            ...
            if isinstance(result_data, list) and len(result_data) > 0:
                self._log(f"💾 [DEBUG] 진행내용 데이터 저장 시작: {len(result_data)}개 행")
            ...
            # 빈 줄 5개 추가
            empty_rows = [['', '', '', ''] for _ in range(config.EMPTY_ROWS_BEFORE_UPDATE)]
            ...
            self._log(f"💾 [DEBUG] save_progress_data 반환 전 - 행 개수: {len(result_data)}")
        ...
        try:
            ...
            requests.append({
                "updateDimensionProperties": {
                    ...
                    "properties": {
                        "pixelSize": config.COLUMN_WIDTH_DATE
                    },
                    "fields": "pixelSize"
                }
            })
            ...
            requests.append({
                "updateDimensionProperties": {
                    ...
                    "properties": {
                        "pixelSize": config.COLUMN_WIDTH_CONTENT
                    },
                    "fields": "pixelSize"
                }
            })
