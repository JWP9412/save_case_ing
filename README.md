...
대법원 나의 사건 조회 자동화 시스템 (Puppeteer + Python GUI) - v4.6.0
...
1. **사건목록 관리 다이얼로그** *(v4.6.0)*
   - 적용·확인·취소 플로우(적용 스냅샷, 취소 시 확인 후 되돌리기, 확인 시 적용+시트 새로고침+닫기). X 버튼 = 취소. 7개 버튼(숨기기, 숨김 해제, 사건 삭제, 변경 취소, 추가, 선택 사건 불러오기, 저장) 선택/폼 상태에 따른 활성·비활성. 람다/리사이저 버그 수정.
...
├── config.py                    설정 상수 (APP_VERSION = "4.6.0" 등)
...
상세 변경 이력: [00.CHANGELOG/CHANGELOG_v4.6.0.md](00.CHANGELOG/CHANGELOG_v4.6.0.md)  
