...
대법원 나의 사건 조회 자동화 시스템 (Puppeteer + Python GUI) - v4.8.0
...
├── config.py                    설정 상수 (APP_VERSION = "4.8.0" 등)
├── main.py                      진입점 (run_app 또는 run_auto_batch)
├── data/                        설정 및 이력용 JSON 통합 폴더
├── src/                         Puppeteer 자동화 코드
├── services/                    비즈니스 로직 (google_oauth, google_calendar, process_controller 등)
├── gui/                         UI (app_controller, panels, dialogs, utils/batch_actions 등)
...
상세 변경 이력: [00.CHANGELOG/CHANGELOG_v4.8.0.md](00.CHANGELOG/CHANGELOG_v4.8.0.md)  
