...
대법원 나의 사건 조회 자동화 시스템 (Puppeteer + Python GUI) - v4.6.3
...
1. **중복 진행내용 버그 수정 및 중복 오류 제거** *(v4.6.3)*
   - 동일 일자·내용이 여러 행일 때 구글 시트에 반복 저장되던 무한 증식 버그 수정(`filter_new_data` 역순 매칭).
   - 제어 패널 **「🧹 중복 오류 제거」**: 선택 사건 시트에서 일자·내용·결과·공시문이 같은 중복 행 일괄 삭제.
...
├── config.py                    설정 상수 (APP_VERSION = "4.6.3" 등)
...
상세 변경 이력: [00.CHANGELOG/CHANGELOG_v4.6.3.md](00.CHANGELOG/CHANGELOG_v4.6.3.md)  
